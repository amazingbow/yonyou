...
{
    public partial class SupplierDocLineListDQryUIModelAction
    {
        ...
        {
            string filterOpath = "";
            if (this.CurrentPart.NameValues.ContainsKey("SupplierID"))
            {
                long supplierID = long.Parse(this.CurrentPart.NameValues["SupplierID"].ToString());
                filterOpath += string.Format("Supplier.Code in (select A.Code from UFIDA::U9::CBO::SCM::Supplier::Supplier A where A.ID = {0}) ", supplierID);
            }
            else
            {
                if (this.MainView.CurrentFilter.Parameters != null)
                    filterOpath += QryHelper.BeforeGetModelFilter(this.MainView);
            }
            return filterOpath;
        }
