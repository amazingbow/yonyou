...
{
	public partial class ShipSecondBQryUIModelAction 
	{
...
this.OnLoadData_DefaultImpl(sender,e);			
        ...
        internal string CustomFilterOpath_Extend(string filterOpath)
        {
            if (this.MainView.CurrentFilter.Parameters != null)
                filterOpath += " and " + BeforeGetModelFilter(this.MainView);
            return filterOpath;
        }
        private string BeforeGetModelFilter(IUIView MainView)
        {
            string ModelOql = string.Empty;
            ModelOql = MainView.CurrentFilter.OriginalOPath;
            foreach (IUIParameter para in MainView.CurrentFilter.Parameters)
            {
                if (!string.IsNullOrEmpty(MainView.CurrentFilter.OriginalOPath) && para.DataValue != null && !string.IsNullOrEmpty(para.DataValue.ToString()))
                {
                    if (para.DataType == System.Data.DbType.Int64)
                        ModelOql = ModelOql.Replace(para.Name, para.DataValue.ToString());
                    else if (para.DataType == System.Data.DbType.String)
                        ModelOql = ModelOql.Replace(para.Name, "'" + para.DataValue.ToString()
                  + "'");
                    else if (para.DataType == System.Data.DbType.DateTime)
                        ModelOql = ModelOql.Replace(para.Name, "'" + para.DataValue.ToString()

                  + "'");
                    else if (para.DataType == System.Data.DbType.Boolean)
                        ModelOql = ModelOql.Replace(para.Name, "'" + para.DataValue.ToString()

                  + "'");
                    else if (para.DataType == System.Data.DbType.Date)
                        ModelOql = ModelOql.Replace(para.Name, "'" + para.DataValue.ToString()
