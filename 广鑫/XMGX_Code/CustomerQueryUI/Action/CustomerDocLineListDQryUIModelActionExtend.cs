...
{
    public partial class CustomerDocLineListDQryUIModelAction
    {
        ...
        {
            string filterOpath = "";
            if (this.CurrentPart.NameValues.ContainsKey("CustomerID"))
            {
                long customerID = long.Parse(this.CurrentPart.NameValues["CustomerID"].ToString());
                filterOpath += string.Format("Customer.Code in (select A.Code from UFIDA::U9::CBO::SCM::Customer::Customer A where A.ID = {0}) " , customerID);
            }
            else 
            {
