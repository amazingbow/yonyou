...
{
	public partial class LaserLabAddUIModelAction : BaseAction
	{
		private static readonly ILogger logger = LoggerManager.GetLogger(typeof(LaserLabAddUIModelAction));
		//强类型化基类Model属性.
		public new LaserLabAddUIModelModel CurrentModel 
		{
			get {
				return (LaserLabAddUIModelModel)base.CurrentModel;
			}
			set {
		...
		public LaserLabAddUIModelAction(IPart part) : base(part)
		{
		}
		//Model参数的构造,用于测试用例.
		public LaserLabAddUIModelAction(LaserLabAddUIModelModel model) : base(model)
		{
		}
		/// <summary>
		/// Help: 
