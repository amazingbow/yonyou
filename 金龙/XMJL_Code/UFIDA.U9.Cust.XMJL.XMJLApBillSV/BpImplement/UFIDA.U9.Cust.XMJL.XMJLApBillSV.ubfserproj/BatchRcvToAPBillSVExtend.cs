namespace UFIDA.U9.Cust.XMJL.XMJLApBillSV.ubfserproj
{
	...
	/// <summary>
	/// BatchRcvToAPBillSV partial 
	/// </summary>	
	public partial class BatchRcvToAPBillSV 
	{	
		internal BaseStrategy Select()
		{
			return new BatchRcvToAPBillSVImpementStrategy();	
		}		
	}
	...
	/// 
	/// </summary>	
	internal partial class BatchRcvToAPBillSVImpementStrategy : BaseStrategy
	{
		public BatchRcvToAPBillSVImpementStrategy() { }

		public override object Do(object obj)
		{						
			BatchRcvToAPBillSV bpObj = (BatchRcvToAPBillSV)obj;
        ...
        private List<long> CreateAPByItem(BatchRcvToAPBillSV bpObj)
        {
            //按rcv查询立账需要的数据
...
and (A47.[ID] = A48.[ID])  left join [PM_RcvLine_Trl] as A49 on (A49.SysMlFlag = 'zh-CN') and (A.[ID] = A49.[ID])  left join [Base_Organization] as A50 on (A.[SrcDoc_SrcDocOrg] = A50.[ID])  left join [Base_Organization_Trl] as A51 on (A51.SysMlFlag = 'zh-CN') and (A50.[ID] = A51.[ID])  left join [PM_RcvDocType] as A52 on (A9.[RcvDocType] = A52.[ID])
 where   (A.[CurrentOrg] = " + bpObj.SRCOrgID + ")  and A9.[AC] = 1 and A25.[Code] = '" + bpObj.SupplierCode + "' and A.ID in (" + str + ")";
        ...
        private List<APDrawBillYDTOData> DisposalDS(DataSet dsTemp, BatchRcvToAPBillSV bpObj)
        {
            ...
            foreach (DataRow rec in dsTemp.Tables[0].Rows)
            {
                ...
                foreach (CBO.FI_SCM.DTOs.APDrawBillYDTOData item in resultDTODataSet)
                {
                    ...
                    {//可以按需求修改
                        ...
                        accrueDocDTOData.APAccrueTaxDetailDTO.ForEach(i => i.CurrentTaxMoney = i.TaxMoney);
                    }
                }
