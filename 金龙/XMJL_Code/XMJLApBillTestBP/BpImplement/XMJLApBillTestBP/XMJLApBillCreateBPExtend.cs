namespace UFIDA.U9.Cust.XMJL.XMJLApBillTestBP
{
    ...
    /// <summary>
    /// XMJLApBillCreateBP partial 
    /// </summary>	
    public partial class XMJLApBillCreateBP
    {
        internal BaseStrategy Select()
        {
            return new XMJLApBillCreateBPImpementStrategy();
        }
    }
    ...
    /// 
    /// </summary>	
    internal partial class XMJLApBillCreateBPImpementStrategy : BaseStrategy
    {
        public XMJLApBillCreateBPImpementStrategy() { }

        public override object Do(object obj)
        {
            XMJLApBillCreateBP bpObj = (XMJLApBillCreateBP)obj;

            var data = CreateAPByItem(bpObj);
            if (!(data != null && data.Count > 0)) return null;
            return data[0];
        }
        private List<long> CreateAPByItem(XMJLApBillCreateBP bpObj)
        {
            //按rcv查询立账需要的数据
...
and (A47.[ID] = A48.[ID])  left join [PM_RcvLine_Trl] as A49 on (A49.SysMlFlag = 'zh-CN') and (A.[ID] = A49.[ID])  left join [Base_Organization] as A50 on (A.[SrcDoc_SrcDocOrg] = A50.[ID])  left join [Base_Organization_Trl] as A51 on (A51.SysMlFlag = 'zh-CN') and (A50.[ID] = A51.[ID])  left join [PM_RcvDocType] as A52 on (A9.[RcvDocType] = A52.[ID])
 where   (A.[CurrentOrg] = " + bpObj.SrcOrgID + ")  and A9.[AC] = 1 and A25.[Code] = '" + bpObj.SupplierCode + "' and A.ID in (" + str + ")";
        ...
        private List<APDrawBillYDTOData> DisposalDS(DataSet dsTemp, XMJLApBillCreateBP bpObj)
        {
