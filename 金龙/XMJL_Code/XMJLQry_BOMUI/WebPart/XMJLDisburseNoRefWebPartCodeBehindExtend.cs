...
{
    public partial class XMJLDisburseNoRefWebPart
    {
		...
		private void FindButton_Click_Extend(object sender, EventArgs  e)
		{
            ...
            if (this.Model.Views[0].Records.Count > 1)
            {
                for (int i = 0; i < this.Model.Views[0].Records.Count - 1; i++)
                {
                    if (this.Model.Views[0].Records[i]["DescFlexField_PrivateDescSeg1"].ToString() == this.Model.Views[0].Records[i + 1]["DescFlexField_PrivateDescSeg1"].ToString())
                    {
                        this.Model.Views[0].Records.Remove(this.Model.Views[0].Records[i + 1]);
                        i = i - 1;
                    }
                }
            }
