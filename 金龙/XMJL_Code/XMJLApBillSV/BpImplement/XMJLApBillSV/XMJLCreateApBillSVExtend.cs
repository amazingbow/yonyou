namespace UFIDA.U9.Cust.XMJL.XMJLApBillSV
{
    ...
    /// <summary>
    /// XMJLCreateApBillSV partial 
    /// </summary>	
    public partial class XMJLCreateApBillSV
    {
        internal BaseStrategy Select()
        {
            return new XMJLCreateApBillSVImpementStrategy();
        }
    }
    ...
    /// 
    /// </summary>	
    internal partial class XMJLCreateApBillSVImpementStrategy : BaseStrategy
    {
        public XMJLCreateApBillSVImpementStrategy() { }

        public override object Do(object obj)
        {
            XMJLCreateApBillSV bpObj = (XMJLCreateApBillSV)obj;
            var data = CreateAPByItem(bpObj);
            if (!(data != null && data.Count > 0)) return null;
            return data[0];
        }
        private List<long> CreateAPByItem(XMJLCreateApBillSV bpObj)
        {
            //按rcv查询立账需要的数据
...
'' as [DocType_Name], 0 as [BusinessType], 0 as [IsTaxPrice], 0 as [NoneTaxPrice], 0 as [Price], 0 as [MatchPUAmount], 0 as [MatchTUAmount], 0 as [PUAmount], 0 as [TUAmount], 0 as [TBUAmount], 0 as [NoneTaxMoney], 0 as [TaxMoney], 0 as [Money], 0 as [TotalFee], A.[EvaluationPricePU] as [EstimatePriceMnyAC],
0 as [MatchNoneTaxMoney], 0 as [IsCanMod_Money], 0 as [IsCanMod_AccrueAmount], 0 as [IsCanMod_MatchAmount], 0 as [IsCanMod_TaxSchedule],
0 as [CurrentNoneTaxPrice], 0 as [CurrentPrice], 0 as [CurrentPUAmount], 0 as [CurrentTUAmount], 0 as [CurrentTBUAmount], 0 as [CurrentMatchAmount], 0 as [CurrentNoneTaxMoney],
0 as [CurrentTaxMoney], 0 as [CurrentMoney], A15.[Round_Precision] as [PriceUOM_Round_Precision], A15.[Round_RoundType] as [PriceUOM_Round_RoundType], A15.[Round_RoundValue] as [PriceUOM_Round_RoundValue], A19.[Round_Precision] as [TradeUOM_Round_Precision],
...
and (A47.[ID] = A48.[ID])  left join [PM_RcvLine_Trl] as A49 on (A49.SysMlFlag = 'zh-CN') and (A.[ID] = A49.[ID])  left join [Base_Organization] as A50 on (A.[SrcDoc_SrcDocOrg] = A50.[ID])  left join [Base_Organization_Trl] as A51 on (A51.SysMlFlag = 'zh-CN') and (A50.[ID] = A51.[ID])  left join [PM_RcvDocType] as A52 on (A9.[RcvDocType] = A52.[ID])
 where   (A.[CurrentOrg] = " + bpObj.SrcOrgID + ")  and A9.[AC] = 1 and A25.[Code] = '" + bpObj.SupplierCode + "' and A.ID in (" + str + ")";
            ...
            //立账
            List<long> apBillHeadIDSet = CreateAPBill(dsTemp, resultDTODataSet);
            string apbillids = "";
            foreach (long id in apBillHeadIDSet)
            {
                apbillids += id.ToString() + ",";
            }
            if (apbillids.Length > 0)
            {
                apbillids = apbillids.Remove(apbillids.Length - 1, 1);
            }
            //using (UFSoft.UBF.Business.ISession session = UFSoft.UBF.Business.Session.Open())
            //{
            //    APBillHead.EntityList aplist = APBillHead.Finder.FindAll("ID in (" + apbillids + ")");
            //    foreach (APBillHead head in aplist)
            //    {
            //        //保存后修改其他信息
            //    }
            //    session.Commit();
        ...
        private List<APDrawBillYDTOData> DisposalDS(DataSet dsTemp, XMJLCreateApBillSV bpObj)
        {
                ...
                foreach (APDrawBillYDTOData data in resultDTODataSet)
                {
                    if (myID == data.LineID)
                    {
                        ...
                        else
                        {
                            ...
                            foreach (CBO.FI_SCM.DTOs.APAccrueTaxDetailDTOData taxDTOData in data.APAccrueTaxDetailDTO)
                            {
                                //计算税
                                var CurrentTaxMoney = cur.MoneyRound.GetRoundValue(ConvertToDecimal(row["CurrentMoney"]) * taxDTOData.TaxRate);
                                totalTax += ConvertToDecimal(CurrentTaxMoney);
                            }
                            #endregion

                            row["CurrentTaxMoney"] = totalTax;
                            row["CurrentNoneTaxMoney"] = ConvertToDecimal(row["CurrentMoney"]) - totalTax;
        ...
        private List<long> CreateAPBill(DataSet dsTemp, List<APDrawBillYDTOData> resultDTODataSet)
        {
            #region 获取上游来源单据信息
            ...
            foreach (DataRow rec in dsTemp.Tables[0].Rows)
            {
                ...
                accrueDocDTOData.IsCanMod = ConvertToBoolean(rec["IsCanMod_AccrueAmount"]);
                accrueDocDTOData.IsCanModMoney = ConvertToBoolean(rec["IsCanMod_Money"]);
                accrueDocDTOData.IsModMatchAmount = ConvertToBoolean(rec["IsCanMod_MatchAmount"]);
                //accrueDocDTOData.IsCanModMatchAmount = ConvertToBoolean(rec["IsCanMod_MatchAmount"]);
                accrueDocDTOData.IsTaxScheduleEnable = ConvertToBoolean(rec["IsCanMod_TaxSchedule"]);
                #endregion
            ...
            // 获取生单方式
            CBO.FI_SCM.DTOs.MakeBillModeEnumData makeBillMode = CBO.FI_SCM.DTOs.MakeBillModeEnumData.FromBill;
            // 获取过滤条件配置信息
            ...
            badProxy.MakeBillMode = (int)makeBillMode;
        ...
        private APQryConfigureParaDTOData GetAllocConfigParaDTOData()
        {
            ...
            dtoData.IsOrg = false;
            dtoData.IsSrcOrg = false;
            dtoData.IsBizOrg = false;
            dtoData.IsAccrueBy = false;
            dtoData.IsAC = false;
            dtoData.IsSrcBusinessType = false;
            #region 可配置条件
            dtoData.IsSettleOrg = false;
            dtoData.IsAccrueSupp = false;
            dtoData.IsAccrueSuppSite = false;
