...
namespace UFIDA.U9.Cust.XMJLBomChangeBE.TestSuite{
	...
	/// <summary>
	/// Cust_Xmjl_MO_BomChange Auto Test Class
	/// </summary>
	[TestFixture]
	public partial class Cust_Xmjl_MO_BomChangeTest{
		/// <summary>
		/// test Create
		/// </summary>
		//[Test]
		public void Cust_Xmjl_MO_BomChangeCRUD() {
		/*	using (TransactionScope scope = new TransactionScope())
			{
				...
				using (ISession session = Session.Open()) {
					#region __merge CreateCust_Xmjl_MO_BomChange
										obj  = UFIDA.U9.Cust.XMJLBomChangeBE.Cust_Xmjl_MO_BomChange.Create() ;
					//add you custome assign code here ...
					#endregion

					Assert.IsNotNull(obj, " Create <" + typeof(Cust_Xmjl_MO_BomChange).ToString() + "> failed.");
					session.Commit();
				}
				...
				using (ISession session = Session.Open()) {
					#region __merge RemoveCust_Xmjl_MO_BomChange	
					obj.Remove();
