...
namespace UFIDA.U9.Cust.XMJLBomChangeBE
{
    ...
    public partial class Cust_Xmjl_MO_BomChange
    {
        public override DocType DocType
        {
            get 
            {
                this.ChangeDocType = Cust_Xmjl_DocumentType.Finder.Find("Code='02'");
                if (this.ChangeDocType == null)
                {
                    throw new Exception("请去缺件表设置代码02的单据类型,来自动生成变更单单号！");
                }
		...
		*/
        ...
        {
            if (this.Cust_Xmjl_MO_BomChangeItem.Count == 0)
                throw new Exception("缺件列表料单为空，请检查！");
            if (this.Cust_Xmjl_MO_BomChangeManufactureNo.Count == 0)
                throw new Exception("缺件变更车号为空，请检查！");
            base.OnValidate();
        ...
        private static Cust_Xmjl_MO_BomChange CreateDefault_Extend()
        {
        ...
        private static Cust_Xmjl_MO_BomChange CreateDefaultComponent_Extend()
        {
            //TODO delete next code and add your custome code here
            throw new NotImplementedException();
        }
