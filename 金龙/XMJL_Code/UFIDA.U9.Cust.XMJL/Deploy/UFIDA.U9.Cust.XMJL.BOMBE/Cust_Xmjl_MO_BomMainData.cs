	...
	public partial class Cust_Xmjl_MO_BomMainData : UFIDA.U9.Base.Doc.DocData
	{
	    ...
	    public static IList<Type> GetKnownTypes()
        {
            IList<Type> knownTypes = new List<Type>();
                            knownTypes.Add(typeof(UFIDA.U9.Cust.XMJL.BOMDocTypeBE.Cust_Xmjl_DocumentTypeData));
		...
		private UFSoft.UBF.Business.BusinessEntity.EntityKey m_bOMDocumentType_SKey ;
		/// <summary>
		/// 单据类型 序列化Key属性。（传递跨组织序列列字段）
		/// 缺件发料主表.Misc.单据类型
		/// </summary>
		[DataMember(IsRequired=false)]
		public UFSoft.UBF.Business.BusinessEntity.EntityKey BOMDocumentType_SKey
		{
			get 
			{
				return m_bOMDocumentType_SKey ;					
			}
			set
			{
				 m_bOMDocumentType_SKey = value ;	
			}
		}
		/// <summary>
		/// 单据类型
		...
		/// </summary>
		[DataMember(IsRequired=false)]
		public System.Int64 BOMDocumentType
		{
			get	
			{	
				if (BOMDocumentType_SKey == null)
					return UFSoft.UBF.Business.Entity.EmptyObjectValue ;
				else
					return BOMDocumentType_SKey.ID ;
			}
			set	
			{	
				if (value == 0 || value == UFSoft.UBF.Business.Entity.EmptyObjectValue )
					BOMDocumentType_SKey = null ;
				else
				{
					if (BOMDocumentType_SKey == null )
						BOMDocumentType_SKey = new UFSoft.UBF.Business.BusinessEntity.EntityKey(value,"UFIDA.U9.Cust.XMJL.BOMDocTypeBE.Cust_Xmjl_DocumentType") ;
					else
						BOMDocumentType_SKey.ID = value ;
				}
			}
		}
