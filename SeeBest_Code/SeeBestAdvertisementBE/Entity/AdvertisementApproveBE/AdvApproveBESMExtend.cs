...
namespace UFIDA.U9.Cust.SeeBestAdvertisementBE.AdvertisementApproveBE
{
    ...
    {
        public partial class AdvApproveBEStateMachine
        {
            ...
            private State OpenLState_SubmitEventDrivenImp(UFIDA.U9.Cust.SeeBestAdvertisementBE.AdvertisementApproveBE.SubmitEvent ev)
            {
            ...
            private State ApprovedState_UndoApproveEventDrivenImp(UFIDA.U9.Cust.SeeBestAdvertisementBE.AdvertisementApproveBE.UndoApproveEvent ev)
            {
                State toState = CurrentState;
                //TODO:实现转移条件
                if (this.Entity.DocType.ConfirmType == UFIDA.U9.Base.Doc.ConfirmTypeEnum.ApproveFlow)
            ...
            private void ChangeEntityState()
            {
                switch (this.CurrentState)
                {
                    case State.OpenLState:
                        this.Entity.DocStatus = AdvAppStatusEnum.OpenL;
                        break;
                    case State.ApprovingState:
                        this.Entity.DocStatus = AdvAppStatusEnum.Approving;
                        break;
                    case State.ApprovedState:
                        this.Entity.DocStatus = AdvAppStatusEnum.Approved;
                        break;
                }
            }
        }
            ...
            if (args.Length == 1)
            {
                ...
                if (ev != null)
                {
                    if (this.EntityKey.ID == ev.EntityKey.ID)
                    {
                        ...
                        {
                            AdvApproveBE entity = this.EntityKey.GetEntity() as AdvApproveBE;
