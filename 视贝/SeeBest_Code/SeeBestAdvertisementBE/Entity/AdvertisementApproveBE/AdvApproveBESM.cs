...
{
	public partial class AdvApproveBE:IStateMachineEntity
	{
		private AdvApproveBEStateMachine _statemachineInstance;
		private static object _lockObject=new object();
		public AdvApproveBEStateMachine StateMachineInstance
		{
			get
			{
				if (_statemachineInstance == null)
				{
					lock(_lockObject)
					{
						if(_statemachineInstance==null)
						{
							_statemachineInstance = new AdvApproveBEStateMachine(this);
						}
					}
				}
		...
		public partial class AdvApproveBEStateMachine:EntityStateMachine
		{
			public AdvApproveBEStateMachine(AdvApproveBE entity)
			{
				_entity = entity;
			}
			public enum State
			...
			#region Properties
			private AdvApproveBE _entity;
			protected AdvApproveBE Entity
			{
				get
				{
					return _entity;
			...
			public void OpenLState_SubmitEventDriven(UFIDA.U9.Cust.SeeBestAdvertisementBE.AdvertisementApproveBE.SubmitEvent ev)
			{
				...
				else 
				{
				    ...
				    {
				        UFSoft.UBF.PL.EntityBeenModifiedException e = new UFSoft.UBF.PL.EntityBeenModifiedException("f1d91c11-fb12-49af-b6a4-c52297a1fd77","已经被其他人操作并保存，当前操作不能保存。");
				        throw e;
				    }
				}
			}
						
			public void ApprovingState_ApprovedEventDriven(UFIDA.U9.GeneralEvents.ApprovalResultEvent ev)
			{
				if (CurrentState == State.ApprovingState)
				{
					SetState(ApprovingState_ApprovedEventDrivenImp(ev),ev);
					PublishEvent(ev);
				}
				else 
				{
			...
			public void ApprovedState_UndoApproveEventDriven(UFIDA.U9.Cust.SeeBestAdvertisementBE.AdvertisementApproveBE.UndoApproveEvent ev)
			{
